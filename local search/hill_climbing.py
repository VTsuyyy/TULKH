...
def find_conflicts(schedule, class_info, room_capacities):
    assigned_classes = set()
    teacher_schedule = {}
    room_schedule = {}

    for id, cls in enumerate(schedule):
        cls_id, start_slot, room = cls
        t, g, s = class_info[cls_id - 1]
        ...
        if not valid:
            return id
    return random.randint(0, len(schedule) - 1)
...
def initial_solution_greedy(N, M, class_info, room_capacities):
    ...
    for cls_id in sorted_classes:
        ...
        if best_slot is None:
            best_slot = random.randint(1, 60 - t + 1)
            best_room = random.randint(1, M)
...
def local_search(N, M, class_info, room_capacities, max_iterations=1000):
...
best_value, best_assigned, final_schedule = local_search(N, M, class_info, room_capacities)
