...
def local_search(N, M, class_info, room_capacities, initial_schedule, max_iterations=1000):
    current_schedule = initial_schedule[:]
    best_value, best_assigned = evaluate_schedule(current_schedule, class_info, room_capacities)

    for _ in range(max_iterations):
        neighbor = generate_neighbors(current_schedule, class_info, room_capacities)
...
def simulated_annealing(N, M, class_info, room_capacities, initial_temperature=1000, cooling_rate=0.95, max_iterations=500):
    current_schedule = initial_solution_greedy(N, M, class_info, room_capacities)
    best_value, best_assigned = evaluate_schedule(current_schedule, class_info, room_capacities)
    ...
    for _ in range(max_iterations):
        _, _, local_best_schedule = local_search(N, M, class_info, room_capacities, current_schedule, max_iterations=50)
        
        neighbor = generate_neighbors(local_best_schedule, class_info, room_capacities)
        current_value, current_assigned = evaluate_schedule(local_best_schedule, class_info, room_capacities)
        neighbor_value, neighbor_assigned = evaluate_schedule(neighbor, class_info, room_capacities)
        
        if neighbor_value > current_value or random.uniform(0, 1) < math.exp((neighbor_value - current_value) / temperature):
            current_schedule = neighbor
            current_value = neighbor_value
            current_assigned = neighbor_assigned
    ...
    return best_value, best_assigned, best_schedule
...
def evaluate_schedule(schedule, class_info, room_capacities):
    ...
    for cls in schedule:
        ...
        # Kiểm tra các slot liên tiếp mà lớp này chiếm dụng
        ...
        if valid:
            # Đánh dấu lịch của giáo viên và phòng học trong các slot chiếm dụng
            for slot in range(start_slot, start_slot + t):
...
best_value, best_assigned, best_schedule = simulated_annealing(N, M, class_info, room_capacities)
best_value, best_assigned = evaluate_schedule(best_schedule, class_info, room_capacities)
